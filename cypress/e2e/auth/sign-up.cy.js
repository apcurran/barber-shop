...
describe("user sign up flow", () => {
    it("signs up a user as Bob Doe", () => {
        ...
        cy.get("form").within(() => {
            cy.get("input[id=first-name]").type("Bob");

            cy.get("input[id=last-name]").type("Doe");

            cy.get("input[id=phone-number]").type("555-555-5555");

            cy.get("input[type=email]").type(Cypress.env("testUserEmail"));

            cy.get("input[id=password]").type(Cypress.env("testUserPassword"));

            cy.get("input[id=repeat-password]").type(
